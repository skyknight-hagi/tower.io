...
같은 타워·같은 성급 위에 드롭하면 그 자리에서 합쳐짐.

- `tryMove` / `tryPlace`가 대상이 같으면 `tryStack`. 결과는 올려둔 칸에 남음.
- 합성 가능 칸은 초록 하이라이트. 5성은 3개가 있어야 드롭 합성이 되고, 그 전에 3개면 기존처럼 자동 승급.

다음에: 벤치 타워를 필드 같은 성급 위에 올려도 합쳐짐.

## 2026-09-21 — 드래그 이동 / 5성 자동 합치기

필드 타워를 드래그로 옮기고, 5성 3개가 모이면 바로 승급.

- 캔버스에서 잡은 타워를 빈 칸에 놓으면 `tryMove`. 짧게 탭하면 선택.
- 5성 3개(같은 종류)가 모이면 `promoteFives`가 자동 합침. 머지 목록에는 4성까지만 뜸.

다음에: 벤치에서 필드로 바로 드래그 배치는 아직 탭 후 칸 지정.

## 2026-09-21 — 6성은 보이되 안내는 5성까지

합치면 6★로 보이게 하고, 미리 알려주지는 않음.

- 벤치·인스펙터·캔버스는 실제 성급(6★ 포함)을 표시.
- 안내/README는 5성까지. 5성 3개 합치기는 `Merge`만 뜨고 `to 6★`·각성 설명은 없음.

다음에: 5성 3개 합친 뒤에야 6★가 보이는지 보면 됨.
